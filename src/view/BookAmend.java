...
public class BookAmend{
	...
	public BookAmend() {
		...
		jdp.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					textarr[7].setText(sdf.format(date));
		...
		jb.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				for (int i = 0; i < 8; i++) {
					BookAmend.textarr[i].setText(null);
				}
			}
		});
		jp5.add(jb);
		...
		jb.setIcon(new ImageIcon("img/修改.png"));
		jb.setBackground(Color.white);
		if (Prestrain.color == "black") {
			jb.setBackground(Color.gray);
		}
			...
			public void actionPerformed(ActionEvent e) {
				...
				Book book = new Book(textarr[0].getText(),
						textarr[1].getText(), textarr[2].getText(), Integer
								...
								.getText());
				if (new BookManagementDaoImpl().update(book)) {
					text1.setText("修改成功");
				} else {
					text1.setText("修改失败");
				}
