...
public class BookLose {
	...
	public BookLose() {
		...
		text1 = new JTextPane();
		text1.setText("尊敬的用户！图书丢失,请不要慌张，耐心填写图书信息,方便后续管理,在您使用系统时，如需帮助，请联系:\nzhibinhu666@gmail.com");
		...
		jb.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				for (int i = 0; i < 7; i++) {
					BookLose.textarr[i].setText(null);
				}
			}
		});
		jp5.add(jb);
		...
		jb.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				text1.setText("查询到" + bookarr.size() + "条记录,请点击【表格】图标后选择需要挂失的书");
			}
		});
		jp5.add(jb);
	...
	public static void BookCheckInTable() {
		...
		jb=new JButton("挂失");
		jb.setIcon(new ImageIcon("img/Book Management Imgs/挂失.png"));
		jb.setBackground(Color.white);
		if (Prestrain.color == "black") {
			jb.setBackground(Color.gray);
			jb.setForeground(Color.white);
		...
		jb.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if(!"".equals(bookid)){
					if(new BookManagementDaoImpl().delete(bookid)){
						text1.setText("操作成功");
					}else{
						text1.setText("操作失败，请重试");
					}
				}
			}
		});
		...
		table.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if(table.getSelectedRow()!=-1){
					bookid=(String) table.getValueAt(table.getSelectedRow(), 0);
					String bookname=(String) table.getValueAt(table.getSelectedRow(), 1);
					BookLose.text1.setText("选中了编号:【"+bookid+"】,书名:【"+bookname+"】");
				}
			}
		});
